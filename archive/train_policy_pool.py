...
if __name__ == '__main__':
    ...
    server_pool_size = torch.tensor(env_config['server_pool_size']).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % test_freq == 0:
            ...
            for test_dq_idx in range(test_batch):
                test_dq = des.DiffDiscreteEventSystem(network, mu, h, 
                                            inter_arrival_dists, service_dists, 
                                            queue_event_options= queue_event_options,
                                            batch = 1, 
                                            temp = env_temp, seed = test_seed + test_dq_idx,
                ...
                if num_pool > 1:
                    ...
                    lex = lex.unsqueeze(0).repeat(test_dq.batch, 1, 1).to(device)
                else:
                    lex = torch.zeros(test_dq.batch, test_dq.s, test_dq.q).to(device)
            
                obs, state = test_dq.reset(seed = test_seed)
                
                total_cost = torch.tensor([[0.]]).to(device)
                time_weight_queue_len = torch.tensor([[0.]]).to(device)
            ...
            with torch.no_grad():
                for _ in trange(test_T):
                    ...
                    pr = net(batch_queue, batch_time)
                    pr = pr.repeat_interleave(num_pool, dim = 1)

                    # test policy
                    if test_policy == 'sinkhorn':
                        # v, s_bar, q_bar = rt.pad_pool(2*pr + lex, batch_queue.detach(), network = test_dq_batch[-1].network.repeat(test_batch, 1, 1), device = device, server_pool_size = server_pool_size)
                        v, s_bar, q_bar = rt.pad(2*pr + lex, batch_queue.detach(), network = test_dq_batch[-1].network.repeat(test_batch, 1, 1), device = device)
                                                # for server_idx in range(test_dq_batch[-1].s):
                        #     s_bar[:,server_idx] = server_pool_size[server_idx]
                        # for queue_idx in range(test_dq_batch[-1].q):
                        #     q_bar[:,-1] = server_pool_size.sum()
                        ...
                        pr = rt.Sinkhorn.apply(-v, s_bar, q_bar, 
                                                ...
                                                sink_eps,
                                                sink_back_temp,
                                                device)[:,:test_dq_batch[-1].s,:test_dq_batch[-1].q]
                    elif test_policy == 'linear_assigment':
                        v, s_bar, q_bar = rt.pad(pr + lex, batch_queue.detach(), network = test_dq_batch[-1].network, device = 'cpu')
                    ...
                    for test_dq_idx in range(len(test_dq_batch)):
                        obs_batch[test_dq_idx], state_batch[test_dq_idx], cost, event_time = test_dq_batch[test_dq_idx].step(state_batch[test_dq_idx], action[test_dq_idx])
                        total_cost_batch[test_dq_idx] = total_cost_batch[test_dq_idx] + cost
                        time_weight_queue_len_batch[test_dq_idx] = time_weight_queue_len_batch[test_dq_idx] + obs_batch[test_dq_idx][0] * event_time
            ...
            print(f"queue lengths: \t{torch.mean(torch.concat([time_weight_queue_len_batch[test_dq_idx] / state_batch[test_dq_idx].time for test_dq_idx in range(len(test_dq_batch))]), dim = 0)}")
            # print(f"queue lengths: \t{torch.mean(torch.concat(time_weight_queue_len_batch) / state.time, dim = 0)}")
        ...
        if device == 'cpu':
            def policy_plot(obs):
                ...
                if test_policy == 'sinkhorn':
                    ...
                    v, s_bar, q_bar = rt.pad(pr + lex, test_dq_batch[-1].queues.detach(), network = network, device = 'cpu')
                    pr = rt.linear_assignment_batch(v, s_bar, q_bar)
                elif test_policy == 'softmax':
                    pr = pr
                else:
        ...
        for _ in trange(train_T):
            ...
            if train_policy == 'sinkhorn':
                v, s_bar, q_bar = rt.pad_pool(2*pr + lex, queues.detach(), network = dq.network, device = device, server_pool_size = server_pool_size)
                ...
                # print(dq.s, dq.q)
                # for server_idx in range(dq.s):
                #     s_bar[:,server_idx] = server_pool_size[server_idx]
                # for queue_idx in range(dq.q):
                #     q_bar[:,-1] = server_pool_size.sum()
        ...
        # Save checkpoint
        torch.save(net, checkpoint_dir + f'.pt')
