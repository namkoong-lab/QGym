...
if __name__ == '__main__':
    ...
    def train(config=None):
        try:
            ...
            with open(f'./configs/env/{args.env_name}', 'r') as f:
                env_config = yaml.safe_load(f)

            with open(f'./configs/model/ppg.yaml', 'r') as f:
                model_config = yaml.safe_load(f)

            with wandb.init(config=config):
                config = wandb.config

                # Update the model configuration with the sweep parameters
                model_config['policy']['sinkhorn']['temp'] = config.sink_temp
                model_config['env']['straight_through_min'] = config.straight_through_min
                model_config['env']['env_temp'] = config.env_temp
                ...
                for epoch in range(num_epochs):
                    ...
                    if device == 'cpu':
                        ...
                        wandb.log({"switch_plot": wandb.Image(f'{fig_dir}_{epoch}.png')})
    ...
    wandb.agent(args.sweep_id, train, count=50)
