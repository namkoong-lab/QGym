...
class DiffDiscreteEventSystem(gym.Env):
    def __init__(self, network, mu, h, inter_arrival_dists, service_dists, init_time = 0, batch = 1, queue_event_options = None,
                 ...
                 device = "cpu", f_hook = False, f_verbose = False, reset = False, burst_prob = 0, burst_factor = 10):
    ...
    def step(self, state, action):
        ...
        #allocated_work = work
        allocated_work, queue_nonzero_inds, num_allocated = allocator(work, service_times)
        ...
        if delta_q.sum() > 0:
        # burst_prob of delta_q = burst_factor * delta_q
            if self.state.random() < self.burst_prob:
                delta_q = self.burst_factor * delta_q
