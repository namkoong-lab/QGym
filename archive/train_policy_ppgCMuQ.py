...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % test_freq == 0:
            ...
            with torch.no_grad():
                for _ in trange(test_T):
                    ...
                    repeated_queue = obs_batch[0][0].repeat(test_batch,network.shape[-2],1)
                    repeated_network = test_dq_batch[-1].network.repeat(test_batch, 1, 1)
                    repeated_mu = mu.repeat(test_batch, 1, 1)
                    repeated_h = h.repeat(test_batch,network.shape[-2],1)
                    ...
                    pr *= repeated_queue * repeated_h * repeated_network * repeated_mu
        ...
        for _ in trange(train_T):
            ...
            train_repeated_queue = obs_batch[0][0].repeat(1,network.shape[-2],1)
            train_repeated_network = test_dq_batch[-1].network.repeat(1, 1, 1)
            train_repeated_mu = mu.repeat(1, 1, 1)
            train_repeated_h = h.repeat(1,network.shape[-2],1)
            ...
            # server pools
            pr = pr * obs_batch[0][0] * dq.network * mu * train_repeated_h
            pr = pr.repeat_interleave(num_pool, dim = 1)
        ...
        with open(f'./loss/{name}_{model_name}_ppgCMuQ.json', 'w') as f:
