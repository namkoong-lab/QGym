...
if __name__ == '__main__':
    ...
    A = queue_event_options[network.size()[1]:]
    theta = h.float().clone().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % test_freq == 0:
            ...
            with torch.no_grad():
                for _ in trange(test_T):
                    ...
                    pr = F.softmax(dq.mu * torch.sum(-A.unsqueeze(0).repeat(test_batch,1,1) * dq.queues.unsqueeze(1) * g.unsqueeze(0).repeat(test_batch,1,1), 2).unsqueeze(1), -1)
                    pr = pr.repeat_interleave(num_pool, dim = 1)

                    # test policy
                    if test_policy == 'sinkhorn':
                        v, s_bar, q_bar = rt.pad(pr + lex, dq.free_servers.data, dq.queues.data, network = dq.network, device = device)
                    ...
                    obs, state, cost, event_time = dq.step(action)
            ...
            #     def policy_plot(obs):
            #         g = F.softplus(theta)
            #         pr = F.softmax(dq.mu * torch.sum(-A.unsqueeze(0) * obs.unsqueeze(1) * g.unsqueeze(0), 2).unsqueeze(1), -1)

            #         if test_policy == 'sinkhorn':
            #             v, s_bar, q_bar = rt.pad(pr, torch.ones((1,dq.s)), obs, network = network.unsqueeze(0), device = device)
            #             pr = rt.Sinkhorn.apply(-v, s_bar, q_bar, 
            #                                     sink_num_iter,
            #                                     sink_temp,
            #                                     sink_eps,
        ...
        for _ in trange(train_T):
            ...
            pr = F.softmax(dq.mu * torch.sum(-A.unsqueeze(0).repeat(train_batch,1,1) * queues.unsqueeze(1) * g.unsqueeze(0).repeat(train_batch,1,1), 2).unsqueeze(1), -1)
            ...
            if train_policy == 'sinkhorn':
                v, s_bar, q_bar = rt.pad(pr, dq.free_servers.data, dq.queues.data, network = dq.network, device = device)
                pr = rt.Sinkhorn.apply(-v, s_bar, q_bar, 
                                        sink_num_iter,
                                        sink_temp,
                                        sink_eps,
        ...
        torch.nn.utils.clip_grad_norm_([theta], max_norm = model_config['opt']['grad_clip_norm'])
        optimizer.step()

        if not model_config['env']['train_restart']:
            init_train_queues = dq.queues.detach()
        ...
        # Save checkpoint
        torch.save(theta, checkpoint_dir + f'_{epoch}.pt')
        
        with open(f'./loss/{name}_backpressure.json', 'w') as f:
