...
#         arg_max = F.one_hot(torch.argmax(logits, dim = 1), num_classes = logits.size()[1])

#         ctx.save_for_backward(arg_max, logits, y_soft, tau)
#         return arg_max.float()
...
#         grad: torch.Tensor, 
#     ):
#         arg_max, logits, y_soft, tau = ctx.saved_tensors
        
#         shifted_y_soft = .5 * ((logits / tau).softmax(dim=-1) + arg_max)
#         grad_at_input_1 = (2 * grad) * shifted_y_soft
#         grad_at_input_1 = grad_at_input_1 - shifted_y_soft * grad_at_input_1.sum(dim=-1, keepdim=True)
        
#         grad_at_input_0 = (-0.5 * grad) * y_soft
...
class STargmin(nn.Module):
    ...
    def forward(self, x):
        #return ReinMaxCore.apply(-x, self.temp)
