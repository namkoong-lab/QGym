...
class DirectBackpropCMuQPolicy:
    def __init__(self, network):
        self.network = network
        
    def test_forward(self, step, batch_queue, batch_time, repeated_queue, repeated_network, repeated_mu, repeated_h):
        return self.network(batch_queue, batch_time) + (repeated_h * repeated_network * repeated_mu * repeated_queue)
    
    def train_forward(self, queues, time, network, repeated_h, mu):
        # print("ppg out", self.network(queues, time))
        # print("cmuq", network * repeated_h * mu)
        return self.network(queues, time) + (network * repeated_h * mu * queues.unsqueeze(1).repeat(1, self.network.s, 1)).detach()
